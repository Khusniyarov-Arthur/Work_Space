...
const toogleFilter = () => {
  if (filterBtn.classList.contains("vacancies__filter-btn-active")) {
    filterBtn.classList.remove("vacancies__filter-btn-active");
    filter.classList.add("filter__visible");
  } else {
    filterBtn.classList.add("vacancies__filter-btn-active");
    filter.classList.remove("filter__visible");
  }
};

filterBtn.addEventListener("click", toogleFilter);
...
const init = () => {
  ...
  getData(
    ...
    (locationData) => {
      const locations = locationData.map((item) => ({ value: item }));
      cityChoices.setChoices(locations, "value", "lable", false);
    },
    (error) => {
      console.log(error);
    }
  ...
  const toggleModal = (e) => {
    const vacancyCard = e.target.closest(".vacancy");
  ...
  cardsList.addEventListener("click", toggleModal);
